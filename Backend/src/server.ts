...
let runtime: Awaited<ReturnType<typeof fastify.db.connect>> | undefined;
...
async function connectWithRetry(
  url: string,
  attempt = 1,
): Promise<Awaited<ReturnType<typeof fastify.db.connect>>> {
  try {
    const connection = await fastify.db.connect({ url });
